...
def narrate_map_tiles(tiles: list[lq.TileInfo]) -> str:
    """Format all revealed map tiles, grouped by row (y-coordinate)."""
    if not tiles:
        return "No revealed tiles."

    # Group by y, sort by x within each row
    by_row: dict[int, list[lq.TileInfo]] = {}
    for t in tiles:
