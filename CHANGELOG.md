...
- **Turn report moved into `get_full_game_state`**: In shared (human-vs-agent) games the deferred post-turn report (snapshot diff, threats, warnings) is now delivered as the `=== TURN REPORT ===` section of `get_full_game_state` on the seat's first call after coming back on the clock, instead of as the return value of `wait_for_turn`. `wait_for_turn` now returns a short status pointing the agent to `get_full_game_state`. The report is built once per turn (then consumed) and only when the seat actually holds the local-player slot, so off-clock reads don't stall AI processing.

- **Human vs agent play**: `CIV_MCP_AGENT_PLAYERS=1,2` lets a human keep playing in the Civ 6 UI while several MCP agents run rival civs in the same single-player game. A `GameEvents.PlayerTurnStarted` hook moves the local-player slot between civs at each turn boundary; agents claim a seat (`claim_seat`), read their own empire at all times, and write only on their own turn (`get_turn_status` / `wait_for_turn`). Serves over `streamable-http` so one server process owns the single FireTuner connection. Validated live in a 4-player game through turn 5: ten consecutive local-player switches, per-seat reads while off the clock, and the built-in AI unaffected. See [docs/human-vs-agent.md](docs/human-vs-agent.md).
- **Web dashboard start is non-fatal**: a port clash no longer kills the MCP server. uvicorn calls `sys.exit(1)` when it cannot bind, and the resulting `SystemExit` in a background task was tearing down the whole ASGI app.
